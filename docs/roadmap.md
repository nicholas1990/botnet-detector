...
- [ ] Dashboard opzionale (sez. 11) — decisioni prese, implementazione da fare:
  ...
  - **Meccanismo di condivisione dati: opzione (b) — scelta.** `Detector.run()` è un loop bloccante (`scapy.sniff`) e resta un processo separato, tipicamente lanciato con `sudo` (come oggi via `main.py`); ad ogni finestra chiusa scrive il risultato (`score`, `status`, `reasons`, `indicators`, timestamp) su un file/SQLite condiviso. La dashboard Streamlit gira come processo distinto, senza privilegi elevati, e legge periodicamente quello store per aggiornare la UI.
    - **Perché (b):** principio del privilegio minimo — solo il processo di cattura deve girare come root; il codice web-facing (Streamlit, superficie di attacco più ampia perché espone una porta) resta completamente disaccoppiato e non privilegiato. Beneficio secondario: i due processi hanno cicli di vita indipendenti (si può riavviare la dashboard senza toccare la cattura, e viceversa) e lo storico delle finestre passate resta disponibile su disco.
    - **Accorgimento implementativo:** con SQLite usare il WAL mode, per evitare errori "database is locked" nell'accesso concorrente singolo-scrittore/singolo-lettore.
